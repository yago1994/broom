...
function applyPlant(rule, options) {
  ...
  // Constrain the slot — and therefore the plant inside — to the original
  // swept element's footprint. Falls back to a sensible cap if missing.
...
function pickerStylesheet(cursorValue) {
  return `
    ...
    /* In edit (plant) mode, planted slots show a faint ground patch so the
       user can see where each plant is anchored. */
